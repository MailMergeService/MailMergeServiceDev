...
namespace XServices.Common.ServiceConsumer
{
    ...
    public class ExternalServiceCaller
    {
        ...
        public async Task<T> GetAsync<T>(string service)
        {
            ...
            return await SendRequestAndGetResponse<T>( (httpClient) =>  httpClient.GetAsync(CreateEndPoint(service)));
        }

        public async Task<T> PostAsync<T>(string service, object data)
        {
            Console.WriteLine(BaseAddress + service);
            return await SendRequestAndGetResponse<T>((httpClient) => httpClient.PostAsJsonAsync(CreateEndPoint(service), data));
        }

        public async Task PutAsync(string service, object data)
        {
            Console.WriteLine(BaseAddress + service);
             await SendRequestAndGetResponse<dynamic>((httpClient) => httpClient.PutAsJsonAsync(CreateEndPoint(service), data));
        }

        public async Task DeleteAsync(string service)
        {
            Console.WriteLine(BaseAddress + service);
             await SendRequestAndGetResponse<dynamic>((httpClient) => httpClient.DeleteAsync(CreateEndPoint(service)));
        }
